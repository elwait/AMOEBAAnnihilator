...
def ShiftDisplacementByPBC(annihilator,dispcartcomponent):
    if dispcartcomponent<=-float(annihilator.aaxis)/2:
        dispcartcomponent=dispcartcomponent+float(annihilator.aaxis)
    elif dispcartcomponent>float(annihilator.aaxis)/2:
        dispcartcomponent=dispcartcomponent-float(annihilator.aaxis)
    return dispcartcomponent

def ComputeCOM(annihilator,atomidxtovecdic,atomidxtomassdic):
...
def AddHarmonicRestrainGroupTermsToKeyFile(annihilator,keyfilename,teatherdist):
    group1string='group '+str(1)+' '
    group2string='group '+str(2)+' '
    for num in annihilator.restrainatomgroup1:
        group1string+=str(num)+' '
    keymods.AddKeyWord(annihilator,keyfilename,group1string+'\n')       
    for num in annihilator.restrainatomgroup2:
        group2string+=str(num)+' '
    keymods.AddKeyWord(annihilator,keyfilename,group2string+'\n')

    if annihilator.flatbotrest==False:
        restrainstring='restrain-groups '+str(1)+' '+ str(2)+' '+str(annihilator.distancerestraintconstant)+' '+str(teatherdist)+' '+str(teatherdist)
    else:
        restrainstring='restrain-groups '+str(1)+' '+ str(2)+' '+str(annihilator.distancerestraintconstant)+' '+'0'+' '+str(teatherdist)
    keymods.AddKeyWord(annihilator,keyfilename,restrainstring+'\n')
...
def AverageCOMGroups(annihilator,filename):
    ...
    for framecount in range(t.n_frames):
        ...
        if len(g1atomidxtovecdic.keys())==len(annihilator.restrainatomgroup1) and len(g2atomidxtovecdic.keys())==len(annihilator.restrainatomgroup2):
            g1COM=ComputeCOM(annihilator,g1atomidxtovecdic,g1atomidxtomassdic)
            g2COM=ComputeCOM(annihilator,g2atomidxtovecdic,g2atomidxtomassdic)
...
def AddInitialGroupRestraints(annihilator):
    if annihilator.restrainatomgroup1!=None and annihilator.restrainatomgroup2!=None:
        teatherdist=AverageCOMGroups(annihilator,annihilator.xyzfilename)
        annihilator.restraintdistance=teatherdist
        AddHarmonicRestrainGroupTermsToKeyFile(annihilator,annihilator.configkeyfilename,teatherdist)


def GroupRestraintFreeEnergyFix(annihilator):
    fo=annihilator.distancerestraintconstant
    pi=np.pi
    gasconst=8.314/4184
    temp=annihilator.roomtemp
    kt = temp * gasconst
    stdcon = (1.0*10**27) / (6.02214 * 10**23)
    if annihilator.flatbotrest==True:
        ri=0
        fi=1
    else:
        ri=annihilator.restraintdistance
        fi=annihilator.distancerestraintconstant

    ro=annihilator.restraintdistance
    ...
    dh = dg + temp*ds
    annihilator.rescorrection=dg
    return dg,dh,ds


def ComputeIdealRestraints(annihilator,fxyz):
    ligidx = [_-1 for _ in annihilator.ligandindices]
    rest_rotbond = False
    no_orient = False
    find_rotation_rest(annihilator,fxyz, ligidx, "CA C N", rest_rotbond=rest_rotbond, rotbond_only=no_orient)
...
def find_rotation_rest(annihilator,fxyz, ligidx0, atomnames='CA', rcutoff=0.6, alpha=1.0, rest_rotbond=False, rotbond_only=False):
    try:
        t = md.load_arc(fxyz)
    except IOError:
        t = md.load(fxyz)
...
def find_grp_idx(annihilator,fxyz, ligidx0, atomnames='CA', rcutoff=1.2, alpha=1.0):
    try:
        t = md.load_arc(fxyz)
    except IOError:
        t = md.load(fxyz)
    ...
    protidx0 = t.topology.select('name %s'%(atomnames))
    protidx0 = np.array(sorted(set(protidx0) - set(ligidx)))
    distmat = distance_matrix(t.xyz[-1, ligidx, :], t.xyz[-1, protidx0, :])
    ...
    com_lig = np.mean(t.xyz[-1, list(ligidx), :], axis=0)
    com_lig = t.xyz[-1, [iminlig], :]

    wts0 = np.ones(len(protidx))
    xyz_prot = t.xyz[-1, protidx, :]
    ...
    mask1 = wts1 > 0.4*wtm
    com_p1 = (np.mean(t.xyz[-1, protidx[mask1], :], axis=0)).reshape((1, -1))
    xyz_lig = t.xyz[-1, ligidx, :]
    dists = np.linalg.norm(xyz_lig - com_p1, axis=1)
    imin = np.argmin(dists)

    dcom = np.linalg.norm(com_lig - com_p1)
    dmin = np.linalg.norm(xyz_lig[imin] - com_p1)
    grp1indices=[_+1 for _ in protidx[mask1]]
    idx_tinker = write_tinker_idx(grp1indices)
    grp1indices=ConvertRangeSyntaxToNumberList(grp1indices)
    annihilator.restrainatomgroup1=grp1indices 
    outp = ''
    sgrp = ''
    for n in idx_tinker:
        sgrp += ' %5d'%n
        if len(sgrp) > 50 and n > 0:
            outp += ('group 1 %s\n'%sgrp)
            sgrp = ''
    grp2indices=[ligidx[imin] + 1]
    annihilator.restrainatomgroup2=grp2indices 
...
def ComputeIdealGroupRestraints(annihilator,fxyz):
    ligidx = [_-1 for _ in annihilator.ligandindices]
    if len(ligidx) == 0:
    ...
    for r0 in (0.5, 0.6, 0.7):
        for a in (1.0, 0.5, 0.2):
            rmin, outp = find_grp_idx(annihilator,fxyz, ligidx, 'CA C', r0, alpha=a)
            if res is None or rmin < res[0]:
                res = rmin, outp
            if rmin < r_thr:
                break
        if rmin < r_thr:
            #keymods.AddKeyWord(annihilator,annihilator.configkeyfilename,outp)       
            break
    r0_rest = 2.0
    if res[0] >= r0_rest:
    ...
    if annihilator.flatbotrest==False:
        string="restrain-groups 1 2 %f %.3f %.3f"%(annihilator.distancerestraintconstant,min(r0_rest, res[0]+0.5),min(r0_rest, res[0]+0.5))+'\n'

    else:
        string="restrain-groups 1 2 %f 0.0 %.3f"%(annihilator.distancerestraintconstant,min(r0_rest, res[0]+0.5))+'\n'
    #keymods.AddKeyWord(annihilator,annihilator.configkeyfilename,string)       
